...
const AdminQRScanner = () => {
  ...
  const fetchScans = async () => {
    try {
      const res = await axios.get(API_ENDPOINTS.ADMIN.QR_ALL, {
  ...
  const fetchPoints = async () => {
    try {
      const res = await axios.get(API_ENDPOINTS.ADMIN.QR_USER_POINTS(userId), {
        headers: getAuthHeader(),
      });
  ...
  const deleteScan = async (id) => {
    if (window.confirm("Delete this scan?")) {
      try {
        await axios.delete(API_ENDPOINTS.ADMIN.QR_DELETE(id), {
          headers: getAuthHeader(),
        });
        fetchScans();
        fetchPoints();
  ...
  const deleteAllScans = async () => {
    if (window.confirm("Delete all scans?")) {
      try {
        await axios.delete(API_ENDPOINTS.ADMIN.QR_DELETE_ALL_USER(userId), {
          headers: getAuthHeader(),
        });
        setScannedData([]);
        setPoints(0);
